...
namespace TcpAssistant
{
    public class Transform
    {
        ...
        {
            return ToHexString(buffer, index, buffer.Length - index, split);
        }
        ...
        /// <param name="split"></param>
        /// <returns></returns>
        public static string ToHexString(byte[] buffer, int index, int length, string split)
        {
            if (buffer == null || length <= 0) return "";
            
            var str = new StringBuilder();
            int endIndex = Math.Min(index + length, buffer.Length);
            
            for (int i = index; i < endIndex; i++)
            {
                str.Append(String.Format("{0:X2}", buffer[i]));
                if (i < endIndex - 1) // 不在最后一个字节后添加分隔符
                {
                    str.Append(split);
                }
            }
